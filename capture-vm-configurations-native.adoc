...
+
계획 센터에서 VM 인벤토리 데이터를 업로드하고, 이전 인벤토리 데이터 업로드를 확인하고, VM 인벤토리 데이터 세트를 사용하여 마이그레이션을 계획할 수 있습니다. 그런 다음 *AWS 네이티브 컴퓨팅으로 마이그레이션*을 선택합니다.  VMware 마이그레이션 어드바이저 시작 화면이 표시됩니다.
