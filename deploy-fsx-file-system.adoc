...
Workload Factory 계정에 추가한 정책 및 권한에 따라 Workload Factory(읽기/쓰기 모드 사용)를 사용하여 FSx for ONTAP 파일 시스템을 완전히 배포할 수 있습니다.  권한이 적은 경우(읽기 전용 모드) 또는 권한이 없는 경우(기본 모드), Codebox의 CloudFormation 정보를 사용하여 AWS에 FSx for ONTAP 파일 시스템을 직접 배포해야 합니다.
